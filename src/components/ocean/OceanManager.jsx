...
export default function OceanManager() {
    ...
    const { resolution, patchSize, displacementScale, amplitude } = useControls('Ocean Core', {
        ...
        patchSize: {  value: 250.0, min: 100.0, max: 5000.0, step: 100.0 },
        displacementScale: { value: 1.0, min: 0.1, max: 10.0, step: 0.1 },
        amplitude: { value: 0.0001, min: 0.1, max: 1.0, step: 0.001 }
    }, { collapsed: true });
    ...
    const opticsControls = useControls('Ocean Optics', {
        //Normals
        Normals: folder({
            normalScale: { value: 0.01, min: 0.0, max: 0.10, step: 0.0000001 }
        }, { collapsed: true }),
        // Base colors
        Basic: folder({
            waterDeep: { value: '#004f5b' },
            waterShallow: { value: '#008498' },
            colorMinHeight: {  value: -1.0, min: -10.0, max: 0.0, step: 0.1 },
            colorMaxHeight: { value: 0.8, min: 0.0, max: 10.0, step: 0.1 }
        }, { collapsed: true }),
        //SPECULAR
        Specular: folder({
            sunPosition: [0.0, 20.0, -500.0],
            sunColor: { value: '#f5efc4' }, 
            specularPower: { value: 60.0, min: 10.0, max: 300.0, step: 1.0 },
            specularMin: { value: 0.2, min: 0.0, max: 1.0, step: 0.01 },
            specularMax: { value: 0.8, min: 0.0, max: 1.0, step: 0.01 },
            specularIntensity: { value: 1.0, min: 0.0, max: 10.0, step: 0.1 }
        }, { collapsed: true }),

        //Temporary Sky color
        Environment: folder({
            skyColor: { value: '#a7bdd5' }
        }, { collapsed: true }),
